...
namespace Oprazi.Dal.Repositories
{
    public class UserTokenRepository : GenericRepository<UserToken>, IUserTokenRepository
    {
        ...
        public UserTokenRepository(AspNetCoreNTierDbContext aspNetCoreNTierDbContext) : base(aspNetCoreNTierDbContext)
        {
            _aspNetCoreNTierDbContext = aspNetCoreNTierDbContext;
        }

        public async Task<UserTokenResponseModel> GetUserTokenList(UserTokenFilterModel filter)
        {
            var UserTokens = (from usertoken in _aspNetCoreNTierDbContext.UserTokens

                              join user in _aspNetCoreNTierDbContext.Users on usertoken.UserId equals user.Id

                              select new UserTokenListModel
                              {
            ...
            response.Data = UserTokens.Skip(filter.Page != null && filter.Items_Per_Page != null ? (Convert.ToInt32(filter.Page) - 1) * Convert.ToInt32(filter.Items_Per_Page) : 0)
            ...
            if (filter.Items_Per_Page != null)
                response.Payload.Pagination.Last_Page = Convert.ToInt32(Math.Ceiling(UserTokens.Count() / Convert.ToDecimal(filter.Items_Per_Page)));
            ...
            response.Payload.Pagination.Total = UserTokens.Count();

            return response;
        }

        public async Task<UserToken> GetUserTokenById(int Id)
        {
            return (from usertoken in _aspNetCoreNTierDbContext.UserTokens
                    where usertoken.UserId == Id
